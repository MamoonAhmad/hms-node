...
export function EditOutsideRadiologyOrderDialog({ open, onOpenChange, order, patient, onSave }) {
  ...
  // Section 2: Receive radiology report (first encounter or update existing order)
  const [reportDate, setReportDate] = useState('');
  const [reportFile, setReportFile] = useState(null);
  const [performingFacility, setPerformingFacility] = useState('');
  ...
  useEffect(() => {
    if (open && order) {
      ...
      setReportDate('');
      setReportFile(null);
      setPerformingFacility(order.sendOutLocation || '');
      setReportNotes('');
      setReportError('');
  ...
  const handleSave = () => {
    ...
    if (receivingReport && !reportDate.trim()) {
      setReportError('Report date is required when receiving a radiology report.');
      return;
    }
    if (receivingReport && !reportFile) {
      setReportError('Report file is required when receiving a radiology report.');
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="min-w-[800px] max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit outside radiology order</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Attach a referral, requisition, or other document to this order (e.g. when sending out to an external radiology facility).
            </p>
            <input
            ...
            <div>
              ...
              <Input
                placeholder="e.g. Referral for outside radiology"
          ...
          {/* Section 2: Receive radiology report */}
          <div className="space-y-3 border-t pt-4">
            <h4 className="font-medium flex items-center gap-2">
              <FileText className="h-4 w-4" />
              Receive radiology report
            </h4>
            <p className="text-sm text-muted-foreground">
              First encounter: patient has their external radiology report. Second encounter: order was created from this clinic — receive and attach the report for this order.
            </p>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                <Label>Performing facility / imaging center</Label>
                <Input
                  placeholder="External radiology facility name"
                  value={performingFacility}
                  onChange={(e) => setPerformingFacility(e.target.value)}
                  className="mt-1"
                />
              </div>
            </div>
