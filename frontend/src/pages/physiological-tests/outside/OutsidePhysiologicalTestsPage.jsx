...
import { loadPhysiologicalStore, getOrdersByPatientIdOutside } from '../physiologicalStore';
...
export function OutsidePhysiologicalTestsPage() {
  ...
  const allRows = useMemo(() => {
    return store.patients
      .map((p) => {
        const patientOrders = getOrdersByPatientIdOutside(store, p.id);
  ...
  const handleViewDetail = (patientId) => {
    navigate(`/physiological-tests/outside/patient/${patientId}`);
  };

  const handlePrintBarcode = (patientId) => {
    navigate(`/physiological-tests/outside/patient/${patientId}/labels`);
  };

  return (
    <div className="space-y-6 p-4 sm:p-6">
      <h1 className="text-2xl font-bold">Outside physiological tests</h1>
      <p className="text-muted-foreground">Physiological tests performed at external facilities; view and reconcile results by patient</p>
      ...
      <Card>
        ...
        <CardContent>
          <DataTable
            ...
            emptyMessage="No outside physiological test orders"
            actions={(r) => (
