...
export function SpecimenCollectionPage() {
  ...
  useEffect(() => {
    labApi.getCollectionList({}).then(({ data }) => setCollectionList(data || []));
  }, []);
  ...
  const handlePrintBarcodes = (group) => {
    // Print barcodes for all lab orders of this patient (one label per test)
